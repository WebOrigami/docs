...
The `eg` command line app is a multi-purpose tool. At its most general level, it lets you easily invoke JavaScript from the command line. It's also specifically designed to work with graphs that can be represented with the [Explorable](/explorable) interface, such as JSON and YAML files, file system folders, and web resources.

This page introduces the basics of `eg` through by demonstrating common, useful actions you can accomplish with `eg`.
...
A common task `eg` can accomplish is unpacking the values of a JSON or YAML file into a folder tree, so let's use that now to obtain some sample files you'll use in the rest of the examples.
...
This should create a new folder called `intro` containing a tree of files. `eg` treated the indicated YAML file as a graph, and the `copy` function read values out of that graph and wrote them into the file system graph. You'll use those files in the examples that follow.

## Use parentheses if you prefer

Note the comma after the URL above — the `copy` command is a pre-installed JavaScript _function_ that takes two arguments, and those arguments need to be separated with a comma. If you prefer, you can wrap the function's arguments in parentheses, but shells often interpret parentheses, you may have to quote them:

```sh
$ eg "copy(https://explorablegraph.org/intros/eg.yaml, files/intro)"
```

`eg` contains a small JavaScript-ish expression parser whose syntax makes parentheses implicit. So you don't have to type them, but you can if you want. There are also cases where the parentheses are necessary, as you'll see in a bit.
...
At this rudimentary level, `eg` is effectively a tool like `cat` for displaying files. But it can do more interesting things.
...
When you ask `eg` to evaluate "greet.js", it looks up that key in the current scope. As a result, it just displays the contents of the indicated JavaScript file.

But when you ask `eg` to evaluate "greet", it can't find a file called "greet". In that case, `eg` looks to see if "greet.js" exists. This time it finds a result. Since the result is a JavaScript module, `eg` dynamically imports that module and looks at the module's default export. That's a function, so `eg` implicitly executes it. The result of the function call is the string "Hello, world.", so `eg` displays that.
...
`eg` accepts strings in single quotes or backticks, but _not_ double quotes. (The double quotes above are parsed by the shell, not `eg`.)
...
`eg` lets you invoke and compose functions any way you like without having to write permanent code. This can be useful when you're experimenting, or just need to do one-off operations from the shell.
...
The `eg` tool is also useful for feeding files to JavaScript functions without making you write code to deal with files.
...
You can easily combine `eg` features like JSON/YAML parsing, path syntax, and function invocation to have `eg` parse a specific value out of a JSON/YAML file and feed that to your function.
...
By default, `eg` renders graphs in YAML format, a superset of JSON which is generally easier for people to read. If you prefer JSON format, you can ask for that with the `json` function:
...
You can also go the other direction by invoking `yaml`:
...
If you open the indicated URL in your browser, you'll be able to browse the nodes of the graph. When you browse to that server, the server will translate HTTP requests into a graph traversal. Press Ctrl+C to stop the server.
...
Template languages are very good at translating data into something you can present to a user. As a bare-bones template language, we can write a function using native JavaScript template literals.
...
We can then apply this template to data, potentially plucked out of a graph:
...
One real power of representing data as explorable graphs is being able to easily transform an entire graph of one of object into a new graph of a different type of object, simply by defining the transformation function that handles a single object.
...
Note that the second argument to `map` is a function. (Technically, it can be any explorable graph, but for our purposes, let's assume it's a regular JavaScript function here.) This means we _don't_ want `eg` to do its normal implicit function invocation here. To prevent that, we need to manually specific parentheses, which generally requires quoting the arguments to `eg` or otherwise escaping them.
...
The most important thing to understand about `map` is that it does _not_ do all its work when invoked. Instead, it immediately returns a new explorable graph that will invoke the mapping function on demand. An explorable graph is essentially a _lazy dictionary_. It only does work when it has to. In the `uppercase` example above, `eg` does end up doing all the `uppercase` work — but only because we're asking `eg` to display the complete results. In other cases, the work will only be done when asked.
...
If you browse to one of the resulting pages (e.g., http://localhost:5000/Alice), you'll see a message like "Hello, Alice." rendered in HTML. That rendering work is _only done on request_.
