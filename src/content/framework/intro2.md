...
To transform our team data into an About Us website, we'll write some formulas in a small expression language. To lay the groundwork for that, let's begin with some "Hello, world" exercises of basic Origami formulas.
...
In the `src` folder, create a new, empty file with the following _name_:
...
Without needing to do anything else, return to the browser window showing your running web site, then navigate to the `src` folder. You should now see a listing that includes:

1. a real file named `team.yaml` with the names you entered.
1. a real file named `message = 'Hello, world!'`. This file is empty.
1. a virtual file named `message` that was generated by the above formula.
...
In the terminal, change to the `src` directory and run the following commands. (If the server is currently running, press Ctrl+C to stop it.)
...
As before, be sure to use single quotes, not double quotes. (To encourage cross-platform compatibility with Microsoft Windows, which does not allow double quotes in file names, Origami doesn't recognize double quotes.)
...
If the server is not running, start it with `ori serve`. Navigate to (or refresh) the `src` route, which should now show an entry for `hello.html`. If you open that page, you'll see "Hello, world!". Each time you ask for `hello.html`, the web server will evaluate the formula you defined.
