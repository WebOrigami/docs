...
Traverses the `source` [map-based tree](/async-tree/mapBasedTree.html) and writes all values into the `target` tree.
...
The `target` must support a `set` method. The two types of trees defined in the [async-tree](/async-tree) library that provides such support are [ObjectMap](/async-tree/ObjectMap.html) and [FileMap](/async-tree/FileMap.html). Only the latter provides persistent effects, so `copy` is typically used to copy the values from the source tree into file system files.
