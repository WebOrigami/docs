...
Given a [map-based tree](/async-tree/mapBasedTree.html) and a `key`, this returns a child node for the given key, creating it if necessary. This method is called by tree operations like [`Tree.apply`](/builtins/tree/apply.html) and [`Dev.copy`](/builtins/dev/copy.html) when copying subtrees.
