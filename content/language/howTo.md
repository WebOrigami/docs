...
[Cached generated images or other files](#cache-generated-files)<br>
[Sort items in arbitrary order](#sort-arbitrary-order)<br>
...
See the [`Tree.cache`](/builtins/tree/cache.html) documentation for an example of how to cache images generated by a map API.

<a name="sort-arbitrary-order"></a>

## Sort items in arbitrary order

You may have a set of items in one order and want to impose a different order. For example, by default a folder of files will be sorted in [natural order](/builtins/origami/naturalOrder.html) (alphabetically, for the most part), and you may want to render data from those files in a different order.

The [`Tree.sort`](/builtins/tree/sort.html) function can sort items according to the value of a data field or some other expression, but what if you want to impose an arbitrary order?

Suppose you have a `poems` folder with a set of files. As noted above, the default sort order will generally be alphabetic:

```console
$ ori keys poems
${ Origami.yaml(Tree.keys(samples/help/sort/poems)) }
```

To impose an arbitrary order, create an Origami file (in the same folder, if that's convenient) that defines an object whose properties are the files you want in the order you want:

```ori
// ordered.ori
${ samples/help/sort/ordered/ordered.ori }
```

This uses [object property shorthand](/language/expressions.html#shorthand-properties) to define an object whose names are the file names, and whose values are the files' contents.

Then instead of using the folder as the data source, use that object instead:

```console
$ ori keys ordered.ori
${ Origami.yaml(Tree.keys(samples/help/sort/ordered/ordered.ori)) }
```
