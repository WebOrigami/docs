...
<span class="tutorialStep"></span> In the Map Tree pattern discussed later, any function can be treated like a map, and vice versa. This means you can use path syntax as a convenient alternative to specify a string argument:
...
Note the trailing slash in `stdin/`, which invokes the [`stdin`](/builtins/dev/stdin.html) function to obtain the complete standard input. This lets you pipe data to a JavaScript function that accepts a simple argument instead of needing specialize the function specifically to read data from the console.
