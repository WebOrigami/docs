...
With that in place, we can now write code to transform the map of markdown into a corresponding map of HTML.
...
<div class="sideBySide split2to3">
  ...
  <figcaption>Virtual map of HTML</figcaption>
</div>

The markdown map is "real", in the sense that it is persistently stored in some fashion as `object.js`. In the code that follows, we'll create a new map which is _virtual_ as it exists only while the code is running.
...
The transformed map's `keys` method iterates over the keys of the inner `source` map. If a key ends in `.md`, the extension will be replaced with `.html`. For example, if the inner `source` has a key `Alice.md`, the transformed map will return the key `Alice.html`.
...
If the HTML map is asked for something that doesn't end in `.html`, it simply forwards that request to the inner map and returns the result.
...
```${'js'}
/* src/map/HtmlMapTransform.test.js */

${ pattern/map/HtmlMapTransform.test.js }
```

<span class="tutorialStep"></span> From inside the `src/map` directory, run the transform tests:

```console
$ node HtmlMapTransform.test.js
…
# tests 3
# pass 3
# fail 0
...
You can think of this `transform` function as a function that takes a map of markdown and returns a virtual map of HTML.

But the resulting virtual map is actually _lazy_: it does not do any substantive work when it is created. The real work — to enumerate the HTML keys, and to get the values of those keys — will only be performed when we actually traverse the map. This laziness is a vital aspect of the transformation; we can use this map in various ways and be sure we are only doing the minimum work necessary.
